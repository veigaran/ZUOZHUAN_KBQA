...
def params():
    ...
    add_arg("--data_proc_file", default=os.path.join(root_path + "data", "data_proc.pkl"),
            help="file for processed data", type=str)
    add_arg("--emb_file", default=os.path.join(root_path + "data", "vec.txt"), help="Path for pre_trained embedding",
            type=str)

    add_arg("--train_file", default=os.path.join(root_path + "data", "example.train"), help="Path for train data",
            type=str)
    add_arg("--dev_file", default=os.path.join(root_path + "data", "example.dev"), help="Path for dev data", type=str)
    add_arg("--test_file", default=os.path.join(root_path + "data", "example.test"), help="Path for test data",
            type=str)
